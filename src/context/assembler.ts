...
export function assembleContext<TState>(
  ...
  for (const entry of history) {
    ...
    if (providerType === "anthropic") {
      historyMessages.push({
        role: "assistant",
        content: [
          { type: "tool_use", id: callId, name: entry.tool, input: entry.params },
        ],
      });
      historyMessages.push({
        role: "user",
        content: [
          { type: "tool_result", tool_use_id: callId, content: entry.result, is_error: entry.success === false },
        ],
      });
    } else {
      historyMessages.push({
        role: "assistant",
        tool_calls: [
          { id: callId, type: "function", function: { name: entry.tool, arguments: JSON.stringify(entry.params) } },
        ],
      });
      historyMessages.push({
        role: "tool",
  ...
  enforceBudgets(
    { instructions: fullInstructions, state: stateText, history: historyText, tools: toolDescriptions },
    budgets,
    tokenizer
  );
